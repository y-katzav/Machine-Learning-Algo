...
# =============================================================================
#Random frest class baiss on decsion tree algoritem with extra function
# =============================================================================
class RandomForest():
...
#            print('left')
    ...
    def create_subsample(self,n):
        return self.train.sample(n=n)
    
    def split_sample(self,sub_sample):
        j=int(np.sqrt(self.train.shape[1]))
        b=sub_sample.iloc[:,2::].sample(n=j,axis=1)
        a=sub_sample.iloc[:,0:2]
        return pd.concat([a,b],axis=1)
        
    
    def forests_random(self,m):
        sample=self.create_subsample(30)
        forest=[]
        for i in range(m):
            data=self.split_sample(sample)
            forest.append(node(data))
            self.split_node(forest[i])
            
        return forest
    
    def bagging_predict(self,forest,row):
        n=len(forest)
        vo=[]
        for i in range(n):
            vo.append(self.predict(row,forest[i]))
        if(vo.count('M')>vo.count('B')):
            return 'M'
        else:
            return 'B'
...
if __name__ == "__main__":
    dat=Data(df,8/10)
    Algo=RandomForest(dat.train,dat.test)
    
    nod=node(dat.train)
  #  tr=Algo.main_jini_calc(dat.train)
    bagging_predict=[]
    forest=Algo.forests_random(7)
    for i in range(len(dat.test)):
        bagging_predict.append(Algo.bagging_predict(forest,dat.test.iloc[i,:]))
    accurecy=(bagging_predict==dat.test[1]).value_counts()
